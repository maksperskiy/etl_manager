...
import useValidation from "../../modules/validation/useValidation";
import dataSourceService from "../../services/data-source";
import Modal from "../common/Modal/Modal";
import DataSourceForm, {
  DataSourceFormModel,
} from "./components/DataSourceForm/DataSourceForm";
import DateRenderer from "./renderers/DateRenderer";
import { ColumnDef, DataSource, ExtendedRendererProps } from "./types";

import useModalController from "../../hooks/useModalController";
import {
  createModelToFormData,
  dataSourceToEditModel,
  editModelToFormData,
} from "../../mappers/data-source.mapper";
import {
  AnyRequired,
  required,
  ValidateIf,
...
} from "./renderers/ActionsRenderer/ActionsRenderer";
...
export default function DataSources() {
  ...
  const { validate, errors } = useValidation<DataSourceFormModel>({
    name: [required],
    host: ValidateIf(
      (model) =>
        model.source_type === "POSTGRES" || model.source_type === "MYSQL",
      [required],
    ),
    port: ValidateIf(
      (model) =>
  ...
  const handleCreate = async () => {
    if (validate(createModel)) {
      await dataSourceService.postDataSource(
        createModelToFormData(createModel),
      );
  ...
  const handleSave = async () => {
    if (validate(editModel)) {
      await dataSourceService.patchDataSource(
        editModel.pk!,
        editModelToFormData(editModel),
      );
      addModalController.close();
      fetchDataSources();
    }
  ...
  return (
    <div className="etlm-data-sources">
      <Button
        className="etlm-data-sources__add"
        renderIcon={Add}
      ...
      <Table aria-label="sample table">
        <TableHead>
          <TableRow>
            {columnDefs.map((def) => (
              <TableHeader key={def.key}>{def.label}</TableHeader>
            ))}
          </TableRow>
        </TableHead>
        <TableBody>
          {dataSources.map((src) => (
            <TableRow key={src.name}>
              {columnDefs.map((def) => (
                <TableCell className={def.class} key={def.key}>
                  {def.renderer
                    ? def.renderer({
                        dataSource: src,
                        key: def.key,
                        ...(def.props ||
                          ({} as ExtendedRendererProps<ActionsRendererProps>)),
                      })
