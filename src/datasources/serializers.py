...
class DataSourceListSerializer(DataSourceDetailSerializer):
    class Meta:
        model = DataSource
        fields = ["name", "source_type", "author", "created_at", "last_used", "file_ext"]
...
class DataSourceCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data["source_type"] == "FILE" and not data.get('upload'):
            raise serializers.ValidationError("File should be uploaded for 'FILE' source_type.")
        if data.get('upload') and not data["source_type"] == "FILE":
            raise serializers.ValidationError("File can be uploaded only for 'FILE' source_type.") 
        if file := data.get('upload'):
            if file.size > 50 * 1024 * 1024:
                raise serializers.ValidationError("File size cannot exceed 50MB.") 
            if not any([file.name.endswith(f".{ext}") for ext in ['xlsx', 'csv', 'json', 'xml']]):
